...
def _to_Rmesh(plsf, structure,NSx,NSy,NRx,NRy,velocity_ext):
    if velocity_ext == 'HB':
        I = []; CX = []; CY = []
        k_ = 2*NSx/NRx; k=0
        ...
        th_array = structure.th.vector().get_local()
        th_mat = [np.zeros((NRy+1,NRx+1)),np.zeros((NRy+1,NRx+1))]          
        k_ = 2*NSx/NRx; k=0
        for dof in range(0, structure.dofsVvec_max,2):
            if (np.rint(structure.pxvec[dof]) %k_ == .0) and (np.rint(structure.pyvec[dof]) %k_ == .0):
                cx,cy= np.int_(np.rint([structure.pxvec[dof]/k_,structure.pyvec[dof]/k_]))
        ...
        eleComp = np.sum(np.array(structure.Vn.vector()).reshape(-1, 4), axis=1).reshape(NSy,NSx) 
        eleCompLR = np.c_[eleComp[:,0],eleComp]+np.c_[eleComp,eleComp[:,np.size(eleComp,1)-1]];
        nodeComp = (np.r_[eleCompLR,eleCompLR[np.size(eleComp,0)-1,:].reshape(1,NSx+1)]+np.r_[eleCompLR[0,:].reshape(1,NSx+1),eleCompLR])/4
        nodeCompR = nodeComp[::int(NSx/NRx),0::int(NSx/NRx)]
        return nodeCompR

def _to_Rmesh_HV(plsf, structure,NSx,NSy,NRx,NRy):
    th_array = structure.th.vector().get_local()
    th_mat = [np.zeros((NRy+1,NRx+1)),np.zeros((NRy+1,NRx+1))]          
    k_ = 2*NSx/NRx; k=0
    for dof in range(0, structure.dofsVvec_max,2):
