...
class PLSF:
    ...
    def transf_matrix(self,X,Y,nNodes,X_,Y_,nNodes_,k):
        ...
        if k == 1:
            prpX1 = csr_matrix((np.maximum(0,1-r_CS)**3)*(-20*r_CS)*(Ax/r_CS/self.dsp**2),dtype = 'float64')
            prpX2 = csr_matrix((np.maximum(0,1-r_CS)**3)*(-20*r_CS)*(Ay/r_CS/self.dsp**2),dtype = 'float64')
